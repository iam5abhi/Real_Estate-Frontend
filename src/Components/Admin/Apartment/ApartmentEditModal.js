...
export default function ApartmentEditModal({ setOpen, open, GetPropertyData, id }) {
  const cancelButtonRef = useRef(null)
  const [message, setMessage] = useState({ message: '', type: '' })
  const [propertyData, setPropertyData] = useState({ project_name: '',location:'' })
  ...
  const EditSubmitHandler =async (event)=> {
      ...
      try {
        const resp = await authFetch.patch(`/api/admin/property/${id}`,propertyData);
  ...
  const GetPropertyDatas = async () => {
    try {
        const resp = await authFetch(`/api/admin/property/${id}`);
        setPropertyData({ project_name:resp.data.data.project_name,location:resp.data.data.location })
    } catch (error) {
        setMessage({ message: error, type: false })
    }
  }
                  ...
                  <div className="container mx-auto">
                    <div className="col-span-2">
                      <div className=" border-b border-gray-200 rounded">
                        ...
                        <Dialog.Title as="h2" className=" text-xl text-center font-semibold">Edit Apartment</Dialog.Title>
                            ...
                            <div className="relative flex flex-col flex-auto min-w-0 p-4 break-words border-0 shadow-blur rounded-2xl bg-white/80 bg-clip-border mb-2 draggable " draggable="true">
                              <form onSubmit={EditSubmitHandler}>
                                <div >
                                  ...
                                  <input type="text" name='project_name' value={propertyData.project_name} onChange={EditHandleChange} id="title" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-orange-500 focus:border-orange-500 block w-full p-2.5" placeholder='Enter Project Name' required />
                                </div>
                                <div className='mt-1 mb-5'>
                                  ...
                                  <input type="text" name='location' value={propertyData.location} onChange={EditHandleChange} id="description" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-orange-500 focus:border-orange-500 block w-full p-2.5" placeholder='Enter Location' required />
                                </div>

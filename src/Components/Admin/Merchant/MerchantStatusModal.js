...
export default function MerchantStatusModal({setOpen,open,id,GetMerchantData}) {
  ...
  const StatuSubmitHandler = async ()=> {
    try {
      const resp = await authFetch.patch(`/api/admin/get-update-merchant-status/${id}`,{status:status});
                      ...
                      <div className="container w-11/15 mx-auto px-4 bg-white rounded ">
                        <div className="relative flex flex-col flex-auto min-w-0 mt-2 p-4 break-words border-0 shadow-blur rounded-2xl bg-white/80 bg-clip-border mb-4 draggable" draggable="true">
                            ...
                            <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Status <span class="text-red-600">*</span></label>
                             <select id="Status" onChange={(e)=>setStatus(e.target.value)} defaultValue={"status"} name='status' className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-orange-500 focus:border-orange-500 block w-full p-3" >
                            ...
                            <div className='grid justify-items-center'>
                              <button type="button" onClick={StatuSubmitHandler} className="text-white text-end bg-orange-600 hover:bg-orange-400
                                focus:ring-4 focus:ring-orange-300 font-semibold rounded-full text-lg px-12 py-2.5 mr-2 mb-2 focus:outline-none">Update Status</button>
                            </div>
                        </div>
                      </div>

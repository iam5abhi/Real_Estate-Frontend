...
const Projects = () => {
    ...
    const SearchProperties = (event) => {
        ...
        if (event.target.value === "") return propertyData
            return item.propertyname.toLowerCase().includes(event.target.value.toLowerCase())
        })
    ...
    return (
        <>
                ...
                <div className="py-8">
                    <div className='flex justify-between'>
                        ...
                        <div className='flex space-x-2'>
                            ...
                            <button type="button" onClick={() => navigate('/auth/admin/add-project')} className="text-white text-md bg-orange-500 hover:bg-orange-400 focus:ring-4 focus:ring-orange-300 rounded-full px-2.5 py-1.5">
                                <i class="fa-solid fa-plus fa-sm"></i> Add Projects</button>
                        </div> 
                    </div>
                            ...
                            <table className="min-w-full leading-normal">
                                <thead>
                                    ...
                                    <th className="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">
                                        ...
                                        <th className="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">
                                            <i class="fa-solid fa-chart-line"></i> BHK Type
                                        </th>
                                        <th className="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">
                                    ...
                                    {!displayProperty ? null : displayProperty.map((data) => {
                                        return <tr>
                                            <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                                <p className="text-gray-900 whitespace-no-wrap">{data.propertyname}</p>
                                            </td>
                                            <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                                <p className="text-gray-900 whitespace-no-wrap">{data.propertytype}</p> 
                                            </td>
                                            <td className="px-5 py-5 bg-white mt-1.5 text-sm flex">
                                            {data.propertybhk.map(item=> <p className="text-gray-900">&nbsp;{item},</p>)}
                                            </td>
                                            <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                            <p className="text-gray-900 whitespace-no-wrap">{data.price}K</p>
                                            </td>
                                            <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                                <button className={`rounded-full px-3 py-1.5 font-semibold shadow ${data.propertystatus === "active" ? "bg-green-200/75 text-green-900" : "bg-red-200/75 text-red-900"}`}>{data.propertystatus}</button>
                                            </td>
                                            <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm text-center space-x-2">
                                                ...
                                                <button type="button" onClick={() => navigate(`/auth/admin/edit-project/${data._id}`)} className="px-3 py-1.5 font-semibold bg-orange-200 rounded-full shadow">
            ...
            {
              modals.type === "status" ? <ProjectStatusModal setOpen={setModals} id={Id} open={modals} GetPropertyData={GetPropertyData} />: null
            }
        </>
    )
}
